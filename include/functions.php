...
function sb_login($email = '', $password = '', $user_id = '', $user_token = '') {
    ...
    if (isset($ips[$ip]) && $ips[$ip][0] > 5) {
        if ($ips[$ip][1] > time() - 3600) return 'ip-ban';
        unset($ips[$ip]);
...
function sb_add_user($settings = [], $settings_extra = [], $hash_password = true) {
    ...
    if (isset($settings_extra['envato-purchase-code'])) {
        ...
        if ($response !== true) return new SBValidationError('invalid-envato-purchase-code');
    }
                ...
                if (($agent_check && sb_get_setting('notify-agent-email')) || (!empty($user['email']) && (($user_check && sb_get_setting('notify-user-email')) || $conversation['source'] == 'em'))) {
                    if (sb_get_setting('notify-email-cron')) {
                        ...
                        if (!isset($cron_job_emails[$conversation_id])) {
                            $cron_job_emails[$conversation_id] = [$recipient_id, $user_name, $sender['profile_image'], $attachments, sb_isset($conversation, 'department')];
...
function sb_init_translations() {
    global $SB_TRANSLATIONS;
    global $SB_LANGUAGE;
    if (!empty($SB_LANGUAGE) && ($SB_LANGUAGE[0] != 'en' || defined('SB_CLOUD_DEFAULT_LANGUAGE_CODE'))) {
 ...
 * 12. Convert the text formatting of Support Board to HTML
 * 13. Remove the text formatting of Support Board
 * 14. Newsletter
 *
 */
...
function sb_email_get_conversation_code($conversation_id_or_messages) {
    $messages = is_numeric($conversation_id_or_messages) ? sb_db_get('SELECT A.user_id, A.message, A.attachments, A.creation_time, B.first_name, B.last_name, B.profile_image, B.user_type FROM sb_messages A, sb_users B WHERE A.conversation_id = ' . sb_db_escape($conversation_id_or_messages, true) . ' AND A.user_id = B.id ORDER BY A.id ASC', false) : $conversation_id_or_messages;
    ...
    return $code . '<div style="clear:both;"></div></div>';
}
...
function sb_cron_email_notifications() {
    ...
    foreach ($emails as $conversation_id => $value) {
        if ($value) {
            $response = sb_email_create($value[0], $value[1], $value[2], sb_email_get_conversation_code($conversation_id), $value[3], $value[4], $conversation_id);
            if ($response === true) {
    ...
    $emails_to_save = [];
    foreach ($emails as $conversation_id => $value) {
        if ($value) $emails_to_save[$conversation_id] = $value;
    }
